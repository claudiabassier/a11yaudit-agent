...
> **Phase 2 status lives elsewhere.** Started 15 August, now spanning branches `iteration-1-subworkflow-refactor` (renamed from `subworkflow-refactor`, 19 August) and `iteration-2-claude-code`; latest entry **D-93** (19 August, `decision_log.md` v8.1). For current status, read `CLAUDE.md`'s Phase-2 section and `A11yAudit_Fahrplan.md`, not this file. The documentation-status table below (§"Documentation status") lists file versions **as they were at the 7 Aug submission** — every one of them has since moved on; check each file's own version header for the current number, e.g. `decision_log.md` is now v8.1 (D-01 to D-93), not the v3.0 listed below. **This example number has now gone stale three times, exactly as predicted, each time by more entries than the last** (D-68/v5.6 → corrected to D-74/v6.2 by D-77 → found stale again at D-81, six entries later → found stale again at D-93, twelve entries later, this time by a rigorous review rather than routine drift-checking); the instruction to check each file's own header remains the durable part, not any example number written here.
