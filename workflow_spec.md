...
**Version 2.3 · 10 August 2026 — synchronised with the as-built system, Day 6/7 tests and the 10 August pre-commit review folded in**
...
| R4 | `screening_score < 70` | severe issue density → legally risky |
...
- The per-item PEMAT and CCI verdicts *are* produced — by `Validate Output` in SUB-A and `Decision Engine` — and they *are* shown in the generated report, item by item with verdict and rationale. They are simply not persisted as queryable rows; they survive only inside `audits.report_md`.
...
### A4 — `Validate Output` (Code)
1. Parses as JSON (strip stray code fences first).
2. `schema_version`, `summary`, `findings[]`, `instrument_items[]` present.
3. Per finding: required keys; enums valid; `confidence` numeric → clamped [0,1]; `wcag_criterion` matches `^\d\.\d{1,2}\.\d{1,2}$` or null; `evidence` non-empty, ≤300 chars, **and verified to be a literal substring of `content_text`** after whitespace normalisation (anti-fabrication check — a quote that isn't in the source invalidates the finding). Findings that pass are stored with `evidence_verified = true`; findings that fail are dropped and counted in `dropped_unverified`, which is printed in the report. `finding_key` non-empty.
4. Per instrument item: `instrument ∈ {PEMAT, CCI}`, `item_no` within that instrument's valid set, `verdict ∈ {pass, fail, not_applicable}`.
   **Completeness check (review fix #5):** the returned set is diffed against the required item set from the prompt. Missing items become `not_assessed` and are excluded from subscore denominators — otherwise a partial AI response would silently bias the scores. The count of missing items is reported.
5. `findings.length ≤ 25` (truncate, keep most severe).
6. Every AI finding tagged `source: "ai"`.

Valid → A6. Invalid → A5.

*The substring verification in step 3 is the strongest single anti-hallucination control in the system: any finding whose evidence cannot be located verbatim in the source is dropped before it ever reaches the database.*

### A5 — `Repair Attempt` (linear, review fix #4)
No loop-back on the canvas — n8n loops with modified state are fragile and hard to read. Instead a **linear chain**: `AI Analysis` → `Validate` → IF invalid → `AI Analysis (repair)` (second chat node, same config, with the validation errors appended as a repair message) → `Validate (2nd)` → still invalid → fallback. This duplicates one node, a deliberate trade-off of DRY for legibility and testability, recorded in `decision_log.md` D-13. An API error/timeout at either AI node routes directly to the **fallback object**:
```json
