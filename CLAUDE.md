...
| Pfad | Inhalt |
|---|---|
| `workflows_export/` | die drei Pipeline-Workflows wie am 5. August eingereicht - eingefroren, ältere Architektur (vor D-55s Subworkflow-Extraktion), als Beleg behalten, nicht weitergepflegt. `_cheerio_test.json` (Testschrott aus Tag 1, kein Teil der Pipeline) liegt seit D-84 in `archive/day1-cheerio-test/` |
| `workflows_export_dev/` | aktueller Export der lebenden `-dev`-Canvas (4 Workflows, inkl. `SUB-A_Validate-dev`), direkt aus `activeVersionId`/`workflow_history` gezogen - das, wogegen `checks.yml`s `n8n-e2e`-Job und `meta/check_export_sync.py` tatsächlich prüfen (D-89, schließt External-Review-Findings 1 eigentliche Lücke) |
| `.github/workflows/checks.yml` | CI - sechs Jobs bei jedem Push/PR: Secret-Scan, Golden Tests, beide Standalone-Harnesses, statische Checks (Syntax, SQL-Kommentar-Guard, code-vs-export-Sync, zwei advisory Doku-Checks), echter Postgres-Service-Container-Schema-Check, echter Ende-zu-Ende-n8n-Lauf (D-83/D-85/D-87) |
...
| `tools/review_dashboard.py` | rendert `v_review_queue` plus jeden offenen Report in eine einzelne statische HTML-Seite - ein Prüfer öffnet eine Datei, schreibt kein SQL (D-92, schließt External-Review-Finding 3) |
| `meta/check_export_sync.py` | CI-Check - vergleicht jeden gemappten Node-Code in `workflows_export_dev/` gegen `code/*.js`/`*.sql`, Byte für Byte, harter Fail bei Drift (D-92) |
...
- ~~**`instrument_items`** wurde entworfen und aus Zeitgründen gestrichen (Node 15; siehe `decision_log.md` D-14, D-20, D-34). Item-Verdicts stehen im Report, sind aber nicht abfragbar.~~ Erledigt 17. August, `decision_log.md` D-64 - Schreibpfad gebaut (`code/15a_build_instrument_items_payload.js`), Canvas-Verdrahtung Ende-zu-Ende verifiziert (Details unten in der Phase-2-Liste). War hier fälschlich noch als offen geführt, obwohl weiter unten in dieser selben Datei bereits als erledigt vermerkt - Widerspruch gefunden und korrigiert 19. August, `decision_log.md` D-93 (fünfte Review, Fund eines Hintergrund-Subagenten).
...
- ~~**Genauigkeit ist ungemessen.** Kein Vergleich gegen menschliche Auditoren.~~ Teilweise erledigt 17. August, `decision_log.md` D-67 - Hand-Scoring der zwei Fixtures gegen die AI-eigenen Verdikte, 74.5 % kombinierte Übereinstimmung (79.5 % bereinigt, `docs/hand-scoring-comparison.md`). Kein *validierter* Mehr-Gutachter-Vergleich gegen unabhängige menschliche Auditoren - das bleibt offen (`decision_log.md` D-79: "nie im Scope irgendeines einzelnen Findings"); aber "ungemessen" war seit D-67 schlicht falsch, korrigiert 19. August, `decision_log.md` D-93 (derselbe Widerspruch, den D-81 in `readme.md`/`capstone_proposal.md` schon einmal fixte, hier übersehen).
...
- Rigorose Reviews D-77 bis D-81, alle noch am 19. August. *D-77: vier Staleness-Defekte nach D-76 gefunden und gefixt (u. a. dieses Files eigener R4-Eintrag). D-78: Persönliche Inhalte aus der Git-Historie aller drei Branches gepurged. D-79: externe Programmierer-Review - Code-Drift bestätigt, ein echter Safety-Prescreen-Fehlalarm gefixt, ein Paste-Korruptions-Bug gefunden und per direktem Postgres-Write umgangen (erste Abweichung von "immer über die n8n-UI publizieren"). D-80: `finding_key` für KI-Findings jetzt deterministisch statt KI-erfunden - Root Cause der ersten zwei fehlgeschlagenen Live-Deploys war, dass `workflow_entity.nodes` nur der Editor-Entwurf ist, nicht was tatsächlich ausgeführt wird (das ist `workflow_history`, referenziert über `activeVersionId`). D-81: erste rigorose Review über das gesamte Repo (Git, Code-Logik, Konsistenz, Bugs) - drei echte Bugs gefunden und gefixt (`finding_key`-Kollision innerhalb eines Laufs, D-71s Live-SQL-Fix nie ins Repo zurückgeschrieben, PEMAT_17/19 werteten die ganze Seite statt nur den Content-Bereich), plus zehn Doku-Drift-Funde, korrigiert.*
...
**Seither durch Phase 2 überholt, an drei Stellen unten (19. August, drei Konsistenz-Reviews am selben Tag gefunden - die erste rigorose Review, D-81, fand die dritte, die die ersten zwei selbst verpasst hatten):** die Tabellenzahl (`audit_runs` kam mit D-63 dazu, 5 statt 4 - siehe Repository-Tabelle oben, dort schon korrekt), der zitierte R4-Ausdruck - der Drift ist inzwischen größer als beim ersten Fund: nicht mehr nur der fehlende `screening_score !== null &&`-Guard aus D-59, R4 liest seit D-76 (Woche 2, selber Tag) eine ganz andere Variable (`screening_score_deterministic` statt `screening_score`) - und die Behauptung weiter unten, `screening_score_deterministic` existiere "nirgends als Spalte": seit D-63 existiert sie, auf `audit_runs`. Alle drei Zeilen unten bleiben als Snapshot vom 10. August stehen, nicht rückwirkend geändert - für den aktuellen Stand `code/12_decision_engine.js` bzw. `postgres_schema.sql` direkt prüfen, nicht dieses Datum.
