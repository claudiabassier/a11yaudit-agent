...
- **A least-privilege database role exists but is not yet in active use.** `a11yaudit_app` (`postgres_app_role.sql`, `decision_log.md` D-63) is built, granted only the specific statements each table needs, and function-tested with a real password login — but the running workflow's Postgres nodes still authenticate with the original, broader role. The credential switch itself (new n8n credential, re-point Nodes 13/13b/14/15/15a) was planned as a deliberate follow-on step after canvas verification and has not happened yet. n8n's own internal state and the audit tables still share one Postgres role in the system as actually running today.
