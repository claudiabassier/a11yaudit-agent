...
| Path | Purpose |
|---|---|
| `workflows_export/` | the three workflow JSONs as submitted 5 August - frozen, older architecture (predates D-55's subworkflow extraction), kept as evidence, not iterated on |
| `workflows_export_dev/` | a current export of the live `-dev` canvas (4 workflows, including `SUB-A_Validate-dev`), pulled from `activeVersionId`/`workflow_history` - what `checks.yml`'s `n8n-e2e` job and `meta/check_export_sync.py` actually check against (D-89, closing external review Finding 1's real blind spot) |
| `.github/workflows/checks.yml` | CI - six jobs on every push/PR: secret scan, golden tests, both standalone harnesses, static checks (syntax, SQL-comment guard, code/-vs-export sync, two advisory doc checks), a real Postgres service-container schema check, and a real end-to-end n8n run (D-83/D-85/D-87) |
...
| `meta/check_export_sync.py` | CI check - compares every mapped node's code in `workflows_export_dev/` against `code/*.js`/`*.sql`, byte-for-byte, hard failure on drift (D-92) |
