...
- **A screening score of 100 can mean "nothing was checked".** If pasted text is submitted (no markup to check) *and* the AI is unavailable, no criteria are screened at all — and the score defaults to 100, "no issues in screened subset". The statement is literally true and the same report says, three lines below, that all checks were skipped and the AI was unavailable, with all three instrument subscores printed as "not computable". But the score row itself is inconsistent with the rest of its own report, and is a known defect (`decision_log.md` D-36). Safety did not depend on it: that audit still routed to human review, because the deterministic safety prescreen runs before the AI call and fired on its own.
...
- **The fetch-failure path is wired but untested.** A URL that times out or returns an error stops the audit rather than writing a wrong one, but that behaviour has not been executed as a test.
...
- **`legally_relevant` can miss R9-triggered escalations.** It is computed as `R5 OR (R4 AND R1)` — R9 firing alone does not set it, unless the combined score also happens to drop below 70. The decision engine's own bundled test input demonstrates this: R1, R7, R8 and R9 all fire, and `legally_relevant` still comes out `false`. Documented here so a reviewer finds it stated, not discovered.
...
- **No encryption at rest, no access separation beyond the container.** n8n's own internal state and the audit tables share one Postgres role; there is no row-level security and no separate least-privilege database user. `N8N_ENCRYPTION_KEY` protects only n8n's stored credentials (e.g. the Anthropic API key) — it has no bearing on audit content. Anyone who can reach the Postgres port can read everything.
...
2. Print "not computable" instead of 100 when no criteria were screened, matching what the instrument subscores already do (D-36).
