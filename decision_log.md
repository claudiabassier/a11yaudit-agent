...
**Context:** author's own instruction (19 August): personal content unrelated to the project doesn't belong in the project data. Not the first such sweep — Day 1's credential sweep already cleaned `meta/SYSTEM_PROMPT.md`'s bio paragraph and `PROJECT_STATUS.md`'s showcase note under the same rationale — but `CLAUDE.md` was missed at the time. Found on `main`: two spots, one carrying real personal detail beyond a generic reference, neither relevant to the technical project.
