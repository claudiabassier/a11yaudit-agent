...
**Context:** author's own instruction (19 August): personal content unrelated to the project doesn't belong in the project data. Not the first such sweep — Day 1's credential sweep already cleaned `meta/SYSTEM_PROMPT.md`'s bio paragraph and `PROJECT_STATUS.md`'s showcase note under the same rationale — but `CLAUDE.md` was missed at the time. Found on this branch: two spots, one carrying real personal detail beyond a generic reference, neither relevant to the technical project.

**Removed from the current tree and from every commit in history:** a phrase describing `A11yAudit_Onepager.pdf`'s exclusion from the repo (reworded to state only that it isn't a technical artefact); and a clause naming a specific plan that carried personal detail, removed rather than reworded. `git filter-repo --replace-text` with full-phrase rules, same method and same verification (zero hits on the personal terms across all history afterward) as the parallel entries on `main` (its own D-62) and `iteration-2-claude-code` (D-78). A small follow-up fixed one resulting orphaned word, grammatically valid but pointless once the clause it contrasted against was gone.
