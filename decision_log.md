...
**Context:** author's own instruction (19 August): personal content unrelated to the project doesn't belong in the project data. Not the first such sweep — Day 1's credential sweep already cleaned `meta/SYSTEM_PROMPT.md`'s bio paragraph and `PROJECT_STATUS.md`'s showcase note under the same rationale — but `CLAUDE.md` was missed at the time. Found on `main`: two spots, one carrying real personal detail beyond the mere fact of applying (a generic reference, neither relevant to the technical project.

**Removed from the current tree and from every commit in history:** the phrase describing `A11yAudit_Onepager.pdf`'s exclusion from the repo as a "personal-use artefact" (reworded to state only that it isn't a technical artefact); and the entire "Tag 9–11 ([redacted] — Onepager anhängen, Repo-Zugang auf Anfrage, [[redacted] / specific detail, redacted])" clause, removed rather than reworded. `git filter-repo --replace-text` with full-phrase rules, same method and same verification (zero hits on the personal terms across all history afterward) as the parallel entry on `iteration-2-claude-code` (its own D-78). A small follow-up fixed one resulting orphaned word ("direkt Phase 2", grammatically valid but pointless once the clause it contrasted against was gone).
