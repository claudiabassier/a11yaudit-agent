...
4. **`CLAUDE.md`**, "Arbeitsumgebung" section: a paragraph dating from before the very first commit of this sprint still read "kein Commit, kein Tag, kein Branch außer `main`" — false since early in the transfer session (tag `v1.3-capstone`, branch `iteration-1-subworkflow-refactor`, many commits have existed for days). Not related to D-55 specifically, but surfaced by the same sweep; fixed, with the stale target-state bullets struck through rather than deleted, so the paragraph's original planning intent stays legible.
...
## D-62 — Personal content purged from `CLAUDE.md`, including from git history

**Context:** author's own instruction (19 August): personal content unrelated to the project doesn't belong in the project data. Not the first such sweep — Day 1's credential sweep already cleaned `meta/SYSTEM_PROMPT.md`'s bio paragraph and `PROJECT_STATUS.md`'s showcase note under the same rationale — but `CLAUDE.md` was missed at the time. Found on this branch: two spots, one carrying real personal detail beyond the mere fact of applying (a generic reference, neither relevant to the technical project.

**Removed from the current tree and from every commit in history:** the phrase describing `A11yAudit_Onepager.pdf`'s exclusion from the repo as a "personal-use artefact" (reworded to state only that it isn't a technical artefact); and the entire "Tag 9–11 ([redacted] — Onepager anhängen, Repo-Zugang auf Anfrage, [[redacted] / specific detail, redacted])" clause, removed rather than reworded. `git filter-repo --replace-text` with full-phrase rules, same method and same verification (zero hits on the personal terms across all history afterward) as the parallel entries on `main` (its own D-62) and `iteration-2-claude-code` (D-78). A small follow-up fixed one resulting orphaned word ("direkt Phase 2", grammatically valid but pointless once the clause it contrasted against was gone).
