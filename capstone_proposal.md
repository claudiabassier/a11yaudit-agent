...
- **Accuracy has one small data point, not a validated measurement.** A single rater hand-scored PEMAT-P/CCI on two fixtures against the AI's actual verdicts: 74.5% raw agreement, 79.5% excluding one worksheet gap (`decision_log.md` D-67, `docs/hand-scoring-comparison.md`). The schema records every verdict and every human override so a false-positive rate per criterion can eventually be derived from routine use - that remains future work.
