...
> **Phase 2 status lives elsewhere.** Started 15 August, now spanning branches `iteration-1-subworkflow-refactor` (renamed from `subworkflow-refactor`, 19 August) and `iteration-2-claude-code`; latest entry **D-74** (19 August, `decision_log.md` v6.2). For current status, read `CLAUDE.md`'s Phase-2 section and `A11yAudit_Fahrplan.md`, not this file. The documentation-status table below (§"Documentation status") lists file versions **as they were at the 7 Aug submission** — every one of them has since moved on; check each file's own version header for the current number, e.g. `decision_log.md` is now v6.2 (D-01 to D-74), not the v3.0 listed below. **This example number will itself go stale again** — it was last corrected 19 August; the instruction to check each file's own header is the durable part, not the example.
